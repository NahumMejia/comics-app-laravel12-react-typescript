...
function Show({ character, comics }: { character: { id: number; name: string; slug: string }; comics: Comic[] }) {
    return (
        <AppLayout>
            <Head title={`Character: ${character.name}`} />
            <section className="bg-background relative flex flex-1 flex-col gap-8 rounded-2xl p-8 shadow-md dark:bg-gradient-to-b">
                <div className="relative z-10 mb-2">
                    <h1 className="text-foreground text-4xl font-extrabold tracking-tight dark:text-white">
                        Comics related to <span className="text-primary">"{character.name}"</span>
                    </h1>
                </div>
